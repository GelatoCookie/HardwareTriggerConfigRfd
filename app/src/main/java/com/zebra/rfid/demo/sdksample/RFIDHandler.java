...
class RFIDHandler implements Readers.RFIDReaderEventHandler {
    private static final String CONNECTION_FAILED = "Connection failed"; // Move to strings.xml
    private static final String TAG = "RFID_SAMPLE";
    private static final String BUSY_RETRY_MESSAGE = "BUSY and Retry Set Trigger Again!!!"; // Move to strings.xml
    private static final String EPC_TEST_DATA = "111122223333444455556666";
    ...
    private String connectAndConfigureReader() throws InvalidUsageException, OperationFailureException {
        ...
        try {
            ...
            return "Connect OperationFailureException " + e.getStatusDescription();

        } catch (InvalidUsageException e) {
            return "Connect InvalidUsageException " + e.getMessage();
        }
        finally {
    ...
    public boolean setTriggerEnabled(boolean isRfidEnabled) {
        resourceLock.lock();
        try {
            if (!isReadyForTriggerConfig()) return false;
            if (!handleBusyState()) return false;
            return configureTrigger(isRfidEnabled);
        } finally {
            resourceLock.unlock();
        }
    }

    // Helper: Check if reader/context is ready
    private boolean isReadyForTriggerConfig() {
        return reader != null && reader.isConnected() && context != null;
    }

    // Helper: Handle busy state
    private boolean handleBusyState() {
        if (bRfidBusy) {
    ...
    // Helper: Configure trigger and subscribe events
    private boolean configureTrigger(boolean isRfidEnabled) {
    ...
    public void waitForReaderIdle() throws TimeoutException {
        ...
        // Optimize timing for wait for reader idle:
        // poll every 500ms (up to ~5s) instead of a single fixed delay.
    ...
    public class EventHandler implements RfidEventsListener {
        ...
        public void eventStatusNotify(RfidStatusEvents rfidStatusEvents) {
            ...
            if (eventType == STATUS_EVENT_TYPE.HANDHELD_TRIGGER_EVENT) {
                ...
                executor.execute(RFIDHandler.this::disconnect);
    ...
    void testBarcodeInputRfidWriteVerify(){
        ...
        try {
            ...
            throw new IllegalStateException("Timeout waiting for reader idle", e);
        }
        subscribeRfidHardwareTriggerEvents(false);
        setTriggerEnabled(false);
    }
    ...
    public boolean testWriteTag() {
        String writeData = EPC_TEST_DATA;
        String sourceEPC = EPC_TEST_DATA;
        ...
        Runnable progressRunnable = new Runnable() {
            @Override
            public void run() {
                if (context != null && running[0]) {
                    long elapsed = System.currentTimeMillis() - startTime[0];
                    String msg = "Writing tag... " + (elapsed / 1000.0) + "s";
                    context.updateIntegratedTestStatus(msg);
                    progressHandler.postDelayed(this, 200);
                }
            }
        };
        try {
            if (context != null) context.showSnackbar("Writing tag...", false); // Show hourglass UI
            ...
            if (context != null) context.dismissToast(); // Dismiss hourglass UI
    ...
    public boolean verifyWriteTag() {
        ...
        // Use the SyncAPI for previous writeWait so no need to wait
        ...
        Runnable progressRunnable = new Runnable() {
            ...
            public void run() {
                if (context != null && running[0]) {
                    long elapsed = System.currentTimeMillis() - startTime[0];
                    String msg = "Verifying tag... " + (elapsed / 1000.0) + "s";
                    context.updateIntegratedTestStatus(msg);
                    progressHandler.postDelayed(this, 200);
                }
            }
        ...
        try {
            if (context != null) context.showSnackbar("Verifying tag...", false); // Show hourglass UI
            progressHandler.post(progressRunnable);
            TagData tagData = reader.Actions.TagAccess.readWait(sourceEPC, readAccessParams, null, false);
            running[0] = false;
            progressHandler.removeCallbacks(progressRunnable);
            if (context != null) context.dismissToast(); // Dismiss hourglass UI
            if (tagData != null && EPC_TEST_DATA.equals(tagData.getTagID())) {
    ...
    public void verifyWriteTagWithRetry(final int retryCount) {
        if (context == null || reader == null) return;
        Log.i(TAG, "*** verifyWriteTag: reading back to verify");
        Handler progressHandler = new Handler(Looper.getMainLooper());
        final long[] startTime = {System.currentTimeMillis()};
        final boolean[] running = {true};
        ...
        try {
            ...
            if(tagData != null && EPC_TEST_DATA.equals(tagData.getTagID())) {
                Log.d(TAG, "ECRT: Test Passed: Verify");
                context.showSnackbar(context.getString(R.string.test_passed), false);
                restoreToRfidMode();
            } else {
                Log.d(TAG, "ECRT: Test Failed: Verify");
                if (retryCount > 0) {
                    context.showSnackbar(context.getString(R.string.test_failed_retry), true);
                } else {
                    context.showSnackbar(context.getString(R.string.test_failed_no_more_retries), false);
                }
            }
        } catch (InvalidUsageException | OperationFailureException e) {
            running[0] = false;
    ...
    // Helper to restore to RFID mode after test pass
    private void restoreToRfidMode() {
        setTriggerEnabled(true);
        subscribeRfidHardwareTriggerEvents(true);
        bSwitchFromRfidToBarcode = false;
        if (context != null) context.showSnackbar(context.getString(R.string.test_passed_read_write_verify_ok), false);
    }
